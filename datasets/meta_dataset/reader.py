    ...
    def construct_class_datasets(self):
        ...
        for dataset_id in range(self.num_classes):
            ...
            if record_file_pattern.startswith('{}_{}'):
                ...
                data_path = os.path.join(self.base_path, record_file_pattern.format(class_id))
                index_path = os.path.join(self.base_path, index_file_pattern.format(class_id))
            else:
                raise ValueError('Unsupported record_file_pattern in DatasetSpec: %s. '  

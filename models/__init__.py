...
from .protonet_merge import ProtoNetMerge
...
def get_backbone(args):
    if args.arch == 'vit_base_patch16_224_in21k':
        ...
        model = vit.__dict__['vit_small'](patch_size=16, num_classes=0, drop_path_rate=args.drop_path)
    ...
    elif args.arch == 'ibot_small_patch16':
        from . import vision_transformer as vit

        model = vit.__dict__['vit_small'](patch_size=16, num_classes=0)

        if not args.no_pretrain:
            url = "/home/yangxu/project/meta-learning/pmf_cvpr22/pretrained_models/ibot_vitsmall16_pretrain.pth"
            state_dict = torch.load(url)['teacher']
            state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
            state_dict = {k.replace("backbone.", ""): v for k, v in state_dict.items()}
            msg = model.load_state_dict(state_dict, strict=False)
            print('\n\nPretrained weights found at {}, error: {}\n\n'.format(url, msg))
        else:
            state_dict = torch.load(args.pretrained_weights, map_location='cpu')['model']
            state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
            state_dict = {k.replace("backbone.", ""): v for k, v in state_dict.items()}
            msg = model.load_state_dict(state_dict, strict=False)
            print('\n\nPretrained weights found at {}, error: {}\n\n'.format(args.pretrained_weights, msg))

    elif args.arch == 'dino_small_patch16_amt':
        from . import vision_transformer_amt as vit

        model = vit.__dict__['vit_small_amt'](patch_size=16, num_classes=0, r=8, lora_alpha=8, lora_dropout=0.0,
                                              merge_weights=False, init_lora_weights="adsv")
    ...
    elif args.arch == 'dino_small_patch16_amt_merge':
            from . import vision_transformer_amt_merge as vit

            model = vit.__dict__['vit_small_amt_merge'](patch_size=16, num_classes=0,
                                                       lora_moe_r=[8,8,8,8], lora_moe_alpha=[8,8,8,8],
                                                       lora_moe_dropout=0, merge_weights=True)
...
def get_model(args):
    ...
    elif args.deploy == 'merge-vanilla':
        model = ProtoNetMerge(backbone, args=args)
    else:
